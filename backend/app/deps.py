...
@lru_cache
def get_supabase_service() -> LocalDBService:
    # Rename the function later if needed, but keeping it for now minimizes code changes
    settings = get_settings()
    return LocalDBService(settings)
...
async def get_current_user(
    ...
    supabase: LocalDBService = Depends(get_supabase_service),
) -> dict:
