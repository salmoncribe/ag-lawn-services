...
@router.post("/signup")
async def signup(payload: AuthPayload, local_db: LocalDBService = Depends(get_supabase_service)):
    try:
        data = local_db.sign_up(payload.email, payload.password)
        user = data.get("user")
        if user:
            local_db.ensure_profile(user["id"], payload.email)
        return data
...
@router.post("/login")
async def login(payload: AuthPayload, local_db: LocalDBService = Depends(get_supabase_service)):
    try:
        data = local_db.sign_in(payload.email, payload.password)
        user = data.get("user")
        if user:
            local_db.ensure_profile(user["id"], payload.email)
        return data
...
async def twitch_callback(
    ...
    if not user_id:
        raise HTTPException(status_code=400, detail="Invalid state")
    profile = local_db.get_profile(user_id)
    if not profile:
        raise HTTPException(status_code=400, detail="Invalid state")
    ...
    local_db.update_profile(user_id, updates)
